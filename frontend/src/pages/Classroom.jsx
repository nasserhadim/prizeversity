...
// // live classroom-specific balance for THIS user (freshly fetched from server)
...
//         try {
//           // you already mounted /api/xpStudent on the server:
//           // app.use('/api/xpStudent', require('./routes/xpStudent'));
//           const r = await axios.get(`/api/xpStudent/${id}`);
//           // expected shape: { classroom: <id>, xp: number, level: number, ... }
//           if (r?.data && (typeof r.data.xp === 'number') && (typeof r.data.level === 'number')) {
//             setMyCB(r.data);
//           }
//         } catch (e) {
...
// prizeversity/frontend/src/pages/Classroom.jsx
import React, { useState, useEffect } from 'react';
...
const Classroom = () => {
  ...
  useEffect(() => {
    ...
    // NEW: live XP updates — refetch my balance and refresh UI
    ...
    <div className="flex flex-col min-h-screen bg-base-200">
      <div className="flex-grow">
        ...
        <div className="max-w-3xl mx-auto p-6 bg-green-50 rounded-lg space-y-6">
          ...
          {/* XP / Level card (hidden if XP disabled) */}
          {xpSettings?.isXPEnabled === false ? null : (
            <div className="card bg-white border border-base-300 shadow-sm rounded-lg p-4">
              <div className="flex items-center justify-between mb-1">
                <div className="font-semibold">Level {level}</div>
          ...
          {/* Teacher/Admin Controls placeholder */}
          {(user.role === 'teacher' || user.role === 'admin') && (
            <div id="class-settings" className="space-y-4">
              {/* settings UI goes here */}
            </div>
          ...
          {user.role !== 'teacher' && (
            <div className="my-4">
              <button className="btn btn-warning btn-sm" onClick={handleLeaveClassroomConfirm}>
                Leave Classroom
              </button>
            </div>
          )}

          {/* XP test buttons (teacher/admin only) */}
          {(user.role === 'teacher' || user.role === 'admin') && (
            <div className="flex gap-3 mb-6 justify-center">
              <button
                onClick={async () => {
