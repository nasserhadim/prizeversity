...
// ------- XP helpers (mirror backend xpNeededForNextLevel) -------

function xpNeededForNextLevel(level, xpSettings = {}) {
  const xpFormulaType = xpSettings.xpFormulaType || 'exponential';
  const baseXPLevel2 = xpSettings.baseXPLevel2 ?? 100;
...
function computeProgress(xp, level, xpSettings) {
  const need = xpNeededForNextLevel(level, xpSettings);
  const haveRaw = Number(xp || 0);
  const have = Math.max(0, Math.min(need, haveRaw));
...
const StudentStats = () => {
  ...
  const progress = useMemo(() => {
    ...
    return computeProgress(xpState.xp, xpState.level, xpSettings);
  }, [xpState, xpSettings]);
