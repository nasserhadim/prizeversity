...
const InventorySection = ({ userId, classroomId }) => {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <h2 className="text-xl font-bold mb-2">🎒 My Inventory</h2>
      ...
      {items.map((item) => (
        <div key={item._id} className="card flex flex-col md:flex-row gap-4 p-4 bg-base-100 shadow-md">
          ...
          {/* Show student target input only for attack-type items */}
          {['halveBits', 'stealBits'].includes(item.effect) && (
            ...
            >
              <option value="">Select the student to target</option>
              {students
